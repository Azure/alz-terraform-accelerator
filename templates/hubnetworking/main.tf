...
module "hubnetworking" {
  ...
  hub_virtual_networks = {
    primary-hub = {
      name                = "vnet-hub-${var.starter_locations[0]}"
      address_space       = [var.hub_virtual_network_address_prefix]
      location            = var.starter_locations[0]
      resource_group_name = "rg-connectivity-${var.starter_locations[0]}"
      firewall = {
        ...
        default_ip_configuration = {
          public_ip_config = {
            zones = ["1", "2", "3"]
            name  = "pip-hub-${var.starter_locations[0]}"
          }
        }
      }
    }
...
module "virtual_network_gateway" {
  ...
  location              = var.starter_locations[0]
  name                  = "vgw-hub-${var.starter_locations[0]}"
