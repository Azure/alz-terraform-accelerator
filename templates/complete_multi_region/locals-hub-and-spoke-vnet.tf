...
locals {
  ...
  hub_and_spoke_vnet_virtual_json_templated = templatestring(local.hub_and_spoke_vnet_virtual_json_templated, local.config_template_file_variables)
  hub_and_spoke_vnet_virtual_json_final = replace(replace(local.hub_and_spoke_vnet_virtual_json_templated, "\"[", "["), "]\"", "]")
  hub_and_spoke_vnet_virtual_networks = jsondecode(local.hub_and_spoke_vnet_virtual_json_final)
}
