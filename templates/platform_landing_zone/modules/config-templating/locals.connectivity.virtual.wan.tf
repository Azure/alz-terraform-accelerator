locals {
  ...
  virtual_hubs_json           = tostring(jsonencode(var.virtual_hubs))
  virtual_hubs_json_templated = templatestring(local.virtual_hubs_json, local.final_replacements)
  virtual_hubs_json_final     = replace(replace(replace(replace(local.virtual_hubs_json_templated, "\"[", "["), "]\"", "]"), "\"true\"", "true"), "\"false\"", "false")
  virtual_hubs                = jsondecode(local.virtual_hubs_json_final)
}
