/**
 * This Terraform configuration file sets up the infrastructure for a complete virtual WAN deployment.
 * It uses modules to provision resources such as enterprise scale, hub networking, and virtual WAN.
 * The configuration includes various settings and options for customization.
 * 
 * Modules:
 * - `module.enterprise_scale`: Sets up the gouvernance components such as azure policies and management groups
 * - `module.hubnetworking`: Configures a shared virtual network  to be used for jumpbox and bastion.
 * - `module.vwan`: Deploys the virtual WAN infrastructure.
 * 
 * Resources:
 * - `azurerm_virtual_hub_connection.example_connection`: Establishes a connection between the virtual hub and a the shared virtual network.
 * - `azurerm_firewall_policy.this`: Creates a firewall policy for the virtual WAN.
 * - `azurerm_firewall_policy_rule_collection_group.example`: Defines rule collections for the firewall policy.
 * - `azurerm_virtual_machine.bastion`: Deploys a bastion host for secure remote access.
 * - `azurerm_virtual_machine.jumpbox`: Deploys a jumpbox for administrative access.
 * 
 * Note: This code is part of a larger Terraform project and may have dependencies on other files and modules.
 */
...
module "vwan" {
  ...
  allow_branch_to_branch_traffic        = try(local.config.module_vwan.allow_branch_to_branch_traffic, null)
  ...
  er_circuit_connections                = try(local.module_vwan.er_circuit_connections, {})
  expressroute_gateways                 = try(local.module_vwan.expressroute_gateways, {})
  firewalls = {
    for fw_key, fw_value in local.firewalls : fw_key => {
      virtual_hub_key    = fw_value.virtual_hub_key
      name               = fw_value.name
      sku_name           = fw_value.sku_name
      sku_tier           = fw_value.sku_tier
  ...
  p2s_gateway_vpn_server_configurations = try(local.module_vwan.p2s_gateway_vpn_server_configurations, {})
  p2s_gateways                          = try(local.module_vwan.p2s_gateways, {})
  ...
  vpn_gateways                          = try(local.module_vwan.vpn_gateways, {})
  vpn_site_connections                  = try(local.module_vwan.vpn_site_connections, {})
  vpn_sites                             = try(local.module_vwan.vpn_sites, {})
  tags                                  = try(local.module_vwan.tags, {})
...
resource "azurerm_virtual_hub_connection" "example_connection" {
  ...
  virtual_hub_id            =  module.vwan[0].virtual_hub_id[0] 
  remote_virtual_network_id = module.hubnetworking[0].virtual_networks["shared_services"].id
  depends_on = [
    module.vwan,
    module.hubnetworking
  ]
...
module "azure_bastion" {
  ...
  ip_configuration = {
    ...
    subnet_id            =  module.hubnetworking[0].virtual_networks["shared_services"].subnets_name_id["AzureBastionSubnet"]
    ...
    nic1 = {
      ...
      ip_configurations = {
        ipconfig1 = {
          ...
          private_ip_subnet_resource_id =  module.hubnetworking[0].virtual_networks["shared_services"].subnets_name_id["SharedSubnet"]
        }
      }
    }
  }
