...
locals {
  firewalls = { 
    "fw_default" = {
      name               = "fw-avm-vwan",
      sku_name           = local.module_vwan.firewall["fw_default"].sku_name,
      sku_tier           = local.module_vwan.firewall["fw_default"].sku_tier,
      firewall_policy_id = azurerm_firewall_policy.this.id
      tags               = local.module_vwan.firewall["fw_default"].tags,
      virtual_hub_key    = local.module_vwan.firewall["fw_default"].virtual_hub_key
    }
  }
}
...
locals {
  custom_routing_table = {
    name = "rt-bastion", # Change this to the actual name specified in your YAML
    routes = yamldecode(file("${path.module}/config.yaml")).connectivity.vwan.custom_routing_table.rt_bastion.routes
  }
}
